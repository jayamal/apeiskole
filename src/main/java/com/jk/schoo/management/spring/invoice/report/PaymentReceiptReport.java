package com.jk.schoo.management.spring.invoice.report;
...
import com.jk.schoo.management.spring.invoice.domain.DetailItem;
...
public class PaymentReceiptReport {
    ...
    public static StreamResource generate(Transaction transaction) {

        StreamResource streamResource = null;

        List<DetailItem> detailItems = new ArrayList<>();
        detailItems.add(new DetailItem("Customer", transaction.getTransaction().getCustomer().getName()));
        detailItems.add(new DetailItem("Payment Type", transaction.getPaymentType().toString()));
        ...
        detailItems.add(new DetailItem("Payment Date/Time", transaction.getTransactionDate().toString()));
        detailItems.add(new DetailItem("Issuer", "UserName"));
        ...
        try {
            drb .setTitle("# " + transaction.getId())
                ...
                .setSubtitle(transaction.getTransaction().getOffering().getSalesDescription())
            ...
            streamResource = ReportUtil.exportReportPdfAsStream(drb, fileName, dataSource);
        }catch (JRException e) {
            e.printStackTrace();
